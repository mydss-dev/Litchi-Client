...
/// Panels return HTML; render a readable plain-text body in the full reader.
...
/// A single-line title ticker; notice descriptions belong to its full reader.
...
class _V3NoticeBarState extends State<V3NoticeBar>
  ...
  void didUpdateWidget(covariant V3NoticeBar oldWidget) {
    ...
    if (_notices.isEmpty) {
      ...
      final currentId = oldIds.isEmpty ? null : oldIds[_index.clamp(0, oldIds.length - 1)];
  ...
  /// Never include the body: a ticker is navigation, not the announcement reader.
  String _title(NoticeModel notice) =>
      notice.title.trim().isEmpty ? '公告' : notice.title.trim();
            ...
            child: Ink(
              ...
              child: Row(children: [
                Container(
                  ...
                  child: Icon(Icons.campaign_rounded, size: 15, color: p.lycheeInk),
                ),
                ...
                Expanded(child: AnimatedSwitcher(
                  ...
                  child: Text(_title(notice), key: ValueKey(notice.id),
                ...
                if (multiple && MediaQuery.sizeOf(context).width >= 520) ...[
                  ...
                  _NavButton(icon: Icons.chevron_left_rounded,
                    tooltip: '上一条公告', onPressed: () => _goTo(index - 1)),
                  SizedBox(width: 42,
                    child: Text('${index + 1} / ${notices.length}',
                      textAlign: TextAlign.center,
                      style: TextStyle(color: p.inkMuted, fontSize: 10.5))),
                  _NavButton(icon: Icons.chevron_right_rounded,
                    tooltip: '下一条公告', onPressed: () => _goTo(index + 1)),
                ],
                const SizedBox(width: 8),
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 2),
                  child: Text('查看', style: TextStyle(color: p.lycheeInk,
...
class _NavButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final p = V3Palette.of(context);
    return IconButton(
      tooltip: tooltip, onPressed: onPressed, icon: Icon(icon), iconSize: 15,
...
class V3NoticeDialog extends StatefulWidget {
  ...
  static Future<int?> showReader(BuildContext context, {
    required List<NoticeModel> notices, required int initialIndex,
  }) => showDialog<int>(
    context: context,
    barrierColor: Colors.black.withValues(alpha: 0.48),
    builder: (_) => V3NoticeDialog.reader(notices: notices, initialIndex: initialIndex),
  );
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              if (widget.mustRead) ...[
                Container(width: 30, height: 30, alignment: Alignment.center,
                  ...
                  child: Icon(Icons.campaign_rounded, size: 16, color: p.lycheeInk)),
                const SizedBox(width: 12),
              ],
              Expanded(child: Padding(
                padding: EdgeInsets.only(top: widget.mustRead ? 4 : 2),
                child: Text(notice.title.trim().isEmpty ? '公告' : notice.title,
                  maxLines: 2, overflow: TextOverflow.ellipsis,
                  style: TextStyle(color: p.ink, fontSize: 17,
                    fontWeight: FontWeight.w800, height: 1.3)),
              )),
              if (!widget.mustRead) IconButton(tooltip: '关闭',
                onPressed: () => Navigator.of(context).pop(_index),
                icon: const Icon(Icons.close_rounded, size: 20), color: p.inkMuted),
            ]),
          ),
          Flexible(child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 14),
              Text(body.isEmpty ? '（本条公告没有正文）' : body,
          ...
          Padding(
            ...
            child: Row(children: [
              if (multiple) ...[
                _NavButton(icon: Icons.chevron_left_rounded, tooltip: '上一条公告',
                  onPressed: _index > 0 ? () => setState(() => _index--) : null),
                SizedBox(width: 46, child: Text('${_index + 1} / ${widget.notices.length}',
                  textAlign: TextAlign.center,
                  style: TextStyle(color: p.inkMuted, fontSize: 11))),
                _NavButton(icon: Icons.chevron_right_rounded, tooltip: '下一条公告',
...
/// Shows every unseen must-read notice once and records it after dismissal.
