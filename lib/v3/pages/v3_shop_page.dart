    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final cardWidth = columns == 1
            ...
            : (constraints.maxWidth - (compact ? 40 : 68) - gap * (columns - 1)) /
                columns;
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('选择你的线路配额', style: Theme.of(context).textTheme.displayLarge),
                        const SizedBox(height: 8),
                        Text(
                          '周期套餐、一次性套餐与流量包都在这里。购买流程已经完全脱离旧版 UI。',
                    ...
                    for (var i = 0; i < plans.length; i++)
                      SizedBox(
                        ...
                        child: _PlanCard(
                          ...
                          emphasis: plans[i].featured || (i == 0 && plans.length > 1),
...
class _CategoryDeck extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final p = V3Palette.of(context);
    final items = <(PlanCategory?, String, String)>[
      (null, 'All', '全部'),
...
class _PlanCardState extends State<_PlanCard> {
  ...
  Widget build(BuildContext context) {
    final p = V3Palette.of(context);
    final available = _availableCycles(widget.plan);
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: dark ? Colors.white.withValues(alpha: 0.1) : p.panelStrong,
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Text(
                  _categoryLabel(widget.plan.category),
                  style: TextStyle(color: muted, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1.1),
                ),
              ),
              const Spacer(),
              if (widget.plan.hot || widget.emphasis)
                Icon(Icons.auto_awesome_rounded, size: 18, color: dark ? p.cyan : p.accent),
            ],
          ),
          ...
          Text(
            ...
            maxLines: 2,
            overflow: TextOverflow.ellipsis,
            style: TextStyle(color: foreground, fontSize: 21, fontWeight: FontWeight.w850, height: 1.05),
          ),
          const SizedBox(height: 8),
          Text(
            widget.plan.capacity,
            style: TextStyle(color: dark ? p.cyan : p.accent, fontSize: 13, fontWeight: FontWeight.w850),
          ),
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: _cycle == cycle
                            ...
                            : (dark ? Colors.white.withValues(alpha: 0.07) : p.panelStrong),
                        borderRadius: BorderRadius.circular(10),
                      ),
                      child: Text(
                        _cycleLabel(cycle),
                        style: TextStyle(
                          color: _cycle == cycle ? (dark ? p.rail : p.accent) : muted,
                          fontSize: 9,
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('PRICE', style: TextStyle(color: muted, fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: 1.4)),
                    const SizedBox(height: 4),
                    Text(
                      price == null ? '--' : '${widget.currencySymbol}${price.toStringAsFixed(2)}',
                      style: TextStyle(color: foreground, fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: -0.7),
                    ),
                  ],
                ),
              ),
              SizedBox(
                ...
                child: FilledButton.icon(
                  onPressed: widget.plan.soldOut || price == null ? null : () => widget.onBuy(_cycle),
                  ...
                  label: Text(widget.plan.soldOut ? '售罄' : '选择', style: const TextStyle(fontWeight: FontWeight.w850)),
                ),
              ),
            ],
          ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text('当前分类没有可购买套餐', style: TextStyle(color: p.text, fontWeight: FontWeight.w800)),
          const SizedBox(height: 6),
          Text('可以刷新服务端数据后再试。', style: TextStyle(color: p.textMuted, fontSize: 11)),
          const SizedBox(height: 18),
          OutlinedButton(onPressed: onRefresh, child: const Text('刷新套餐')),
        ],
      ),
    );
  }
...
class _V3OrderDialogState extends State<_V3OrderDialog> {
  ...
  int get _discountCents {
    final result = _couponResult;
    if (result == null) return 0;
    if (result.type == 1) return result.value;
    if (result.type == 2) return ((_original * 100) * result.value / 100).round();
    return 0;
  }

  double get _finalPrice => ((_original * 100).round() - _discountCents).clamp(0, 1 << 31) / 100;

  Future<void> _verifyCoupon() async {
    if (_checkingCoupon || _coupon.text.trim().isEmpty) return;
    setState(() {
  ...
  Future<void> _submit() async {
    ...
    try {
      final tradeNo = await widget.api.submitOrder(
        ...
        couponCode: _couponResult == null ? null : _coupon.text.trim(),
      );
      if (!mounted) return;
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('CHECKOUT', style: TextStyle(color: p.accent, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 2)),
                        const SizedBox(height: 7),
                        Text(widget.plan.title, style: Theme.of(context).textTheme.headlineLarge),
                      ],
                    ),
                  ),
              ...
              const SizedBox(height: 22),
              Text('选择周期', style: TextStyle(color: p.textMuted, fontSize: 10, fontWeight: FontWeight.w800)),
              ...
              const SizedBox(height: 22),
              Text('优惠码', style: TextStyle(color: p.textMuted, fontSize: 10, fontWeight: FontWeight.w800)),
              const SizedBox(height: 9),
              Row(
                children: [
                  Expanded(
              ...
              if (_couponResult != null) ...[
                const SizedBox(height: 9),
                Text('优惠码已生效', style: TextStyle(color: p.success, fontSize: 11, fontWeight: FontWeight.w750)),
              ],
              if (_error != null) ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(color: p.panelStrong, borderRadius: BorderRadius.circular(20)),
                child: Row(
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('应付金额', style: TextStyle(color: p.textMuted, fontSize: 10)),
                        const SizedBox(height: 4),
                        Text(
                          '${widget.currencySymbol}${_finalPrice.toStringAsFixed(2)}',
                          style: TextStyle(color: p.text, fontSize: 24, fontWeight: FontWeight.w900),
                        ),
                      ],
                    ),
                    const Spacer(),
                    if (_discountCents > 0)
                      Text(
                        ...
                        style: TextStyle(color: p.success, fontSize: 12, fontWeight: FontWeight.w800),
                      ),
                  ],
                ),
              ),
  ...
  Future<void> _load() async {
    try {
      final results = await Future.wait<Object>([
        ...
        widget.api.getOrderPaymentDetail(widget.tradeNo),
      ]);
      if (!mounted) return;
      final methods = results[0] as List<RemotePaymentMethod>;
      final detail = results[1] as RemoteOrderPaymentDetail;
      setState(() {
        _methods = methods;
        _detail = detail;
      ...
      if (detail.status == 3 || detail.status == 4) await _markPaid();
    } catch (error) {
      if (mounted) {
        setState(() {
          _loading = false;
  ...
  Future<void> _checkout() async {
    ...
    try {
      final result = await widget.api.checkoutOrder(
        widget.tradeNo,
        _balanceOnly && _methods.isNotEmpty ? _methods.first.id : _selectedMethodId,
      );
      if (!mounted) return;
            ...
            : SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('PAYMENT', style: TextStyle(color: p.accent, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 2)),
                              const SizedBox(height: 7),
                              Text('完成支付', style: Theme.of(context).textTheme.headlineLarge),
                            ],
                          ),
                        ),
                        IconButton(onPressed: () => Navigator.of(context).pop(), icon: const Icon(Icons.close_rounded)),
                    ...
                    const SizedBox(height: 8),
                    Text('订单 ${widget.tradeNo}', style: TextStyle(color: p.textMuted, fontSize: 10)),
                    const SizedBox(height: 22),
                    if (_loading)
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            Text('需支付', style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 10)),
                            const Spacer(),
                            Text(
                      ...
                      if (!_balanceOnly && _paymentUrl == null) ...[
                        Text('支付方式', style: TextStyle(color: p.textMuted, fontSize: 10, fontWeight: FontWeight.w800)),
                        const SizedBox(height: 10),
                        if (_methods.isEmpty)
                          Text('当前没有可用支付方式', style: TextStyle(color: p.danger, fontSize: 11))
                        else
                          Wrap(
                            spacing: 8,
                            runSpacing: 8,
                      ...
                      SizedBox(
                        ...
                        child: FilledButton(
                          ...
                          child: Text(
                            _paymentUrl == null
                                ? (_checkingOut ? '正在发起支付…' : _balanceOnly ? '使用余额完成订单' : '继续支付')
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text('套餐数据正在同步到 Litchi。', style: TextStyle(color: p.textMuted, fontSize: 11)),
        const SizedBox(height: 24),
        SizedBox(
          width: double.infinity,
          height: 48,
