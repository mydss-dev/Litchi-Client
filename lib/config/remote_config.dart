...
abstract final class RemoteConfigService {
  ...
  static Future<void> initialize(SharedPreferences prefs) async {
    ...
    final cached = prefs.getString(RemoteConfigSettings.cacheKey);
    if (cached != null) {
      try {
  ...
  static Future<void> _refresh(SharedPreferences prefs) async {
    HttpClient? client;
    try {
      client = HttpClient()..connectionTimeout = RemoteConfigSettings.timeout;
      final request = await client
          .getUrl(Uri.parse(RemoteConfigSettings.configUrl))
          .timeout(RemoteConfigSettings.timeout);
      request.headers.set('Accept', 'application/json');

      final response = await request.close().timeout(RemoteConfigSettings.timeout);
      ...
      await prefs.setString(RemoteConfigSettings.cacheKey, body);
