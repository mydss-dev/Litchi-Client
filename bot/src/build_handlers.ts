...
async function requestBuildConfirmation(
  ...
  if (platforms.length === 0) {
    setPendingAction(userId, { type: "build" });
    await ctx.reply(
      "平台输入不对，请回复 1-4 (1=全部, 2=Windows, 3=Android, 4=macOS)。",
    );
    return;
  }
  const profile = requireBoundProfile(userId);
  ...
  try {
    ...
    if (
      ...
      await ctx.reply(
        "OSS 地址已变化，请先执行 /config 重新生成并上传基础配置。",
      );
      return;
    }
...
async function startBuildFromInput(
  ...
  if (!isAdmin(userId)) {
    ...
    if (recent >= maxBuilds) {
      const resetHint =
        windowHours >= 24 ? "明天再试" : `${windowHours}小时后再试`;
      await ctx.reply(
  ...
  if (startingApps.has(profile.app_id) || hasActiveBuildGroup(profile.app_id)) {
