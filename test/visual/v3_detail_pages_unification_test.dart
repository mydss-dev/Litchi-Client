...
Future<void> pumpPage(
  ...
  await tester.pumpWidget(MaterialApp(
    ...
    themeMode: theme,
    home: AppScope(controller: controller, child: Scaffold(body: page)),
  ));
...
void main() {
  for (final theme in [ThemeMode.light, ThemeMode.dark]) {
    for (final size in [const Size(900, 700), const Size(360, 800)]) {
      testWidgets('node overview is one read-only list at $size $theme',
          (tester) async {
        await pumpPage(tester, const V3NodesPage(), AppPage.nodes,
        ...
        expect(find.descendant(of: list, matching: find.byType(Divider)),
          findsNWidgets(3));
        expect(find.ancestor(of: node, matching: find.byType(InkWell)),
          findsNothing);
        expect(tester.takeException(), isNull);
      });

      testWidgets('compact account retains services and actions at $size $theme',
          (tester) async {
        await pumpPage(tester, const V3AccountPage(), AppPage.account,
          size: size, theme: theme);
  ...
  testWidgets('plan filters have a touch target and a lychee selected state',
      (tester) async {
    await pumpPage(tester, const V3ShopPage(), AppPage.shop,
      size: const Size(360, 800), theme: ThemeMode.light);
