...
class _NewsController extends VisualV3Controller {
  _NewsController({List<NoticeModel> notices = const [], this.update})
      : _live = notices, super(AppPage.dashboard);
...
const _notice = NoticeModel(
  id: 4, title: '服务公告',
  content: '<p>香港、日本线路<b>已完成优化</b>。</p>', createdAt: 1789430400,
);
const _older = NoticeModel(
  id: 3, title: '维护通知', content: '凌晨例行维护。', createdAt: 1789344000,
);
const _popup = NoticeModel(
  id: 9, title: '隐私政策更新', content: '请阅读并同意新的隐私政策。',
  tags: ['弹窗'], createdAt: 1789516800,
);
const _popup2 = NoticeModel(
  id: 10, title: '服务条款更新', content: '请阅读并同意新的服务条款。',
  tags: ['弹窗'], createdAt: 1789603200,
);
const _update = UpdateInfo(
  version: '9.9.9', downloadUrl: 'https://example.com/litchi-9.9.9.exe',
  changelog: '修复了若干问题',
);
...
Future<void> _pumpPage(WidgetTester tester, _NewsController controller, Widget page) async {
  await tester.binding.setSurfaceSize(const Size(900, 700));
  addTearDown(() => tester.binding.setSurfaceSize(null));
  addTearDown(controller.disposeVisual);
  await tester.pumpWidget(AppScope(
    controller: controller,
    child: MaterialApp(theme: V3Theme.light(), home: Scaffold(body: page)),
  ));
  await tester.pump();
}

/// Assert the visible modal title, not a duplicate in the ticker behind it.
/// The homepage now displays the exact title without appending the body.
Finder _dialogTitle(String title) => find.descendant(
  of: find.byType(V3NoticeDialog), matching: find.text(title),
);

void main() {
  testWidgets('the dashboard shows a notice and no banner without one', (tester) async {
    await _pumpPage(tester, _NewsController(notices: const [_notice]),
    ...
    expect(find.textContaining('香港、日本线路'), findsNothing,
        reason: 'notice body must remain in the reader');
    expect(find.textContaining('<b>'), findsNothing);
    expect(tester.takeException(), isNull);
  });

  testWidgets('the notice bar renders nothing when there is no news', (tester) async {
  ...
  testWidgets('the ticker advances through the notices it was given', (tester) async {
    await _pumpPage(tester, _NewsController(notices: const [_notice, _older]),
        const V3DashboardPage());
    expect(find.text('服务公告'), findsOneWidget);
  ...
  testWidgets('the dashboard offers an update and drops it when dismissed', (tester) async {
  ...
  testWidgets('a must-read notice pops once, then is recorded as seen', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(find.byType(V3NoticeDialog), findsOneWidget);
    expect(_dialogTitle('隐私政策更新'), findsOneWidget);
    await tester.tap(find.text('我知道了'));
    await tester.pumpAndSettle();
    expect(find.byType(V3NoticeDialog), findsNothing);
    expect(controller.seen, [9], reason: 'popup must be marked seen');
    expect(tester.takeException(), isNull);
  });

  testWidgets('pending must-read notices are shown one at a time', (tester) async {
    ...
    expect(find.byType(V3NoticeDialog), findsOneWidget);
    expect(_dialogTitle('隐私政策更新'), findsOneWidget);
    expect(_dialogTitle('服务条款更新'), findsNothing);
    await tester.tap(find.text('我知道了'));
    await _frames(tester);
    expect(controller.seen, [9]);
    expect(find.byType(V3NoticeDialog), findsOneWidget);
    expect(_dialogTitle('服务条款更新'), findsOneWidget);
    expect(_dialogTitle('隐私政策更新'), findsNothing);
    await tester.tap(find.text('我知道了'));
    await _frames(tester);
